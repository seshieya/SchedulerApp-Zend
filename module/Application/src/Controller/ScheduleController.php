...
//    protected $startdate;
    ...
    public function draftAction()
    {
        ...
        $generatedDates = $this->_autoGenerateDates($jobAndRowsArray['rowDayInDayOut'], $startdate);
...
//            $data['row'] = $row;
    ...
    private function _autoGenerateDates($daysNeededArray, $startDate) {
        ...
        foreach($daysNeededArray as $key => $daysNeeded) {
            for($i = 0; $i < $daysNeeded; $i++) {
                ...
                if($i == 0) {
                    ...
                    $dayInOutArray[$rowNum . 'dayIn'] = $this->_convertToDateString($seconds);

                    //If $daysNeeded is equal to 1 day, then Day Out would be the same as Day In:
                    if($daysNeeded == ScheduleController::MIN_DAYS_NEEDED) {
                        $dayInOutArray[$rowNum . 'dayOut'] = $dayInOutArray[$rowNum . 'dayIn'];
                    }

                }
                else if($i == ($daysNeeded - 1)) {
                    $seconds = $this->_calculateSeconds($daysIncremented) + $startDateInSeconds;
                    $dayInOutArray[$rowNum . 'dayOut'] = $this->_convertToDateString($seconds);
                }
